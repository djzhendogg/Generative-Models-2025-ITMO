...
def save_samples(tensor, epoch: int):
    fake_images = denorm(tensor)
    fake_images = fake_images.reshape(fake_images.size(0), 3, 64, 64)
    fake_fname = './images/fake_images_new-{0:0=4d}.png'.format(epoch)
    print('Saving', fake_fname)
    save_image(denorm(fake_images), fake_fname, nrow=4)
...
def save_model(configs, path: str):
    ...
    """
    torch.save(
        {'model_state_dict': configs.eps_model.state_dict(), 'optimizer_state_dict': configs.optimizer.state_dict(),
            'n_steps': configs.n_steps, 'image_channels': configs.image_channels, 'n_channels': configs.n_channels,
            'channel_multipliers': configs.channel_multipliers, 'is_attention': configs.is_attention, }, path)
